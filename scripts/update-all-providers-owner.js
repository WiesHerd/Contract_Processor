/**
 * Update All Providers Owner
 * 
 * This script updates all existing providers in the database to have
 * the current user as the owner, making them visible in the app.
 */
...
async function main() {
  try {
    console.log('🚀 Updating All Providers Owner...\n');
    ...
    for (let i = 0; i < providersToUpdate.length; i++) {
      ...
      console.log(`\n[${i + 1}/${providersToUpdate.length}] Updating provider: ${provider.name || provider.employeeId || provider.id}`);
      ...
      if ((i + 1) % 50 === 0) {
        console.log(`\n📊 Progress: ${i + 1}/${providersToUpdate.length} (${Math.round((i + 1) / providersToUpdate.length * 100)}%)`);
      }
    }
